...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    redirect(url('professions/cosmetic-center/doctor/receipt_branding.php'));
}
...
if ($action === 'header') {
    ...
    $upd = $conn_kasher_platform->prepare('UPDATE cosmetic_doctor_accounts SET receipt_header = ?, updated_at = NOW() WHERE id = ? AND user_id = ?');
...
$objectKey = 'img/cosmetic_receipt/user_' . $userId . '_doc_' . $accountId . '_' . time() . '.' . $ext;
...
$upd = $conn_kasher_platform->prepare('UPDATE cosmetic_doctor_accounts SET receipt_logo_url = ?, updated_at = NOW() WHERE id = ? AND user_id = ?');
