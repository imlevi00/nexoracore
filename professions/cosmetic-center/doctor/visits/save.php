...
if ($clientName === '' || $workType === '' || $mobile === '' || $age < 0 || $age > 130 || $sessionsPlanned < 1) {
    ...
    $redir = $recordId > 0 ? url('professions/cosmetic-center/doctor/visits/case.php?id=' . $recordId) : url('professions/cosmetic-center/doctor/visits/new.php');
    redirect($redir);
}

if ($recordId <= 0 && (!is_numeric($priceRaw) || !is_numeric($discountRaw))) {
    setMessage('نرخ و داشکاندنی جەلسەی یەکەم دەبێت ژمارە بن', 'danger');
    redirect(url('professions/cosmetic-center/doctor/visits/new.php'));
}
...
if ($recordId > 0) {
    ...
    if ($sessionsPlanned < $maxS) {
        ...
        redirect(url('professions/cosmetic-center/doctor/visits/case.php?id=' . $recordId));
    }
...
try {
    ...
    $s1->bind_param('issidd', $newId, $firstSessionDate, $role, $doctorAccountId, $price, $discount);
    if (!$s1->execute()) {
        $s1->close();
    ...
    redirect(url('professions/cosmetic-center/doctor/visits/case.php?id=' . $newId));
} catch (Throwable $e) {
