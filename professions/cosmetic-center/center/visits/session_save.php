...
if (!is_numeric($priceRaw) || !is_numeric($discountRaw)) {
    ...
    redirect(url('professions/cosmetic-center/center/visits/case.php?id=' . $caseId));
}
...
$ins = $conn_kasher_platform->prepare('
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())
');
$ins->bind_param('iisssidd', $caseId, $next, $sessionDate, $notes, $role, $centerAccountId, $price, $discount);
if ($ins->execute()) {
