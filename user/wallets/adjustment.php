...
enforceAuthorizationOrDeny($currentUser, 'wallets.adjust', [
    'route' => '/user/wallets/adjustment.php',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Security::validateCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        $currency = (string)($_POST['currency'] ?? 'IQD');
        $amount = (float)($_POST['amount'] ?? 0);
        $direction = ($_POST['direction'] ?? 'in') === 'out' ? 'out' : 'in';
        $type = $direction === 'in' ? 'deposit' : 'withdraw';
        $conn->begin_transaction();
        try {
            $transactionId = walletPostEntry($conn, $userId, $walletId, $type, $direction, $currency, $amount, 'wallet_adjustment', null, (string)($_POST['notes'] ?? ''), $userId);
            if (!$transactionId) {
                throw new Exception('نەتوانرا جوڵەی قاسە تۆمار بکرێت');
            }
            $conn->commit();
            redirect(url('user/wallets/print_receipt.php?id=' . $transactionId));
        } catch (Throwable $e) {
            $conn->rollback();
...
<head>
    ...
    <title>زیاد/کەمکردن - <?php echo SITE_NAME; ?></title>
...
<body class="wallets-module-page wallets-form-page wallets-adjust-page">
<?php include_once '../../includes/navigation.php'; ?>
<div class="container py-4 hub-page-content wa-wrap-narrow">

    <header class="wa-hero">
        <div>
            <div class="wa-kicker"><i class="bi bi-sliders"></i> ڕێکخستنی دەستی</div>
            <h1><i class="bi bi-plus-slash-minus"></i> زیادکردن / کەمکردنی دەستی</h1>
            <p class="wa-hero-sub">باڵانسی قاسە زیاد یان کەم بکە و وەسڵ چاپ بکە</p>
        </div>
    ...
    <section class="wa-panel">
        <div class="wa-panel-head">
            <span><i class="bi bi-pencil-square"></i> وردەکاری جوڵە</span>
        </div>
        <div class="wa-panel-body">
            <form method="post">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <label class="form-label" for="direction">جۆر</label>
                        <select class="form-select" name="direction" id="direction">
                            <option value="in">زیادکردن</option>
                            <option value="out">کەمکردن</option>
                        </select>
                    </div>
                    <div class="col-md-4">
                ...
                <div class="wa-sticky-actions mt-3">
                    <button type="submit" class="wa-btn wa-btn-primary"><i class="bi bi-check-lg"></i> پاشەکەوت</button>
                </div>
            </form>
        </div>
    </section>
