...
$editDoctorId = isset($_GET['edit']) ? (int)$_GET['edit'] : 0;
$doctorForm = [
    'name' => '',
    'email' => '',
    'mobile' => ''
...
if ($editDoctorId > 0) {
    ...
    ");
    if ($editStmt) {
        $editStmt->bind_param('ii', $editDoctorId, $userId);
        $editStmt->execute();
        $editRow = $editStmt->get_result()->fetch_assoc();
        $editStmt->close();
        if ($editRow) {
            $doctorForm['name'] = (string)$editRow['name'];
            $doctorForm['email'] = (string)$editRow['email'];
            $doctorForm['mobile'] = (string)$editRow['mobile'];
        } else {
            $editDoctorId = 0;
            $errors[] = 'پزیشکی داواکراو نەدۆزرایەوە';
        }
    }
}
        ...
        if ($action === 'add' || $action === 'update') {
            ...
            if (empty($errors)) {
                if ($action === 'add') {
                    ...
                    $insertStmt = $conn_kasher_platform->prepare("
                        INSERT INTO doctors (user_id, name, email, password_hash, mobile, created_at, updated_at)
                        VALUES (?, ?, ?, ?, ?, NOW(), NOW())
                    ");
                    if ($insertStmt) {
                        $insertStmt->bind_param('issss', $userId, $name, $email, $passwordHash, $mobile);
                        if ($insertStmt->execute()) {
                            setMessage('پزیشک بە سەرکەوتوویی زیادکرا', 'success');
                            $insertStmt->close();
                            redirect(url('user/medical_staff/doctors.php'));
                        }
                        if ((int)$conn_kasher_platform->errno === 1062) {
                            $errors[] = 'ئەم ئیمەیڵە پێشتر تۆمارکراوە';
                        } else {
                            $errors[] = 'هەڵەیەک لە زیادکردنی پزیشک ڕوویدا';
                        }
                        $insertStmt->close();
                    } else {
                    ...
                    if ($doctorId <= 0) {
                        $errors[] = 'ناسنامەی پزیشک نادروستە';
                    } else {
                        if ($password !== '') {
                            $passwordHash = password_hash($password, PASSWORD_DEFAULT);
                            $updateStmt = $conn_kasher_platform->prepare("
                                ...
                                WHERE id = ? AND user_id = ?
                            ");
                            $updateStmt->bind_param('ssssii', $name, $email, $mobile, $passwordHash, $doctorId, $userId);
                        } else {
                            $updateStmt = $conn_kasher_platform->prepare("
                                UPDATE doctors
                                ...
                                WHERE id = ? AND user_id = ?
                            ");
                            $updateStmt->bind_param('sssii', $name, $email, $mobile, $doctorId, $userId);
                        }

                        if ($updateStmt && $updateStmt->execute()) {
                            setMessage('زانیاری پزیشک نوێکرایەوە', 'success');
                            $updateStmt->close();
                            redirect(url('user/medical_staff/doctors.php'));
                        }
                        if ($updateStmt) {
            ...
            if ($doctorId <= 0) {
                ...
                $checkStmt = $conn_kasher_platform->prepare("
                    SELECT id FROM doctor_secretaries
                    WHERE user_id = ? AND doctor_id = ?
                    LIMIT 1
                ");
                $checkStmt->bind_param('ii', $userId, $doctorId);
                $checkStmt->execute();
                $hasSecretary = $checkStmt->get_result()->num_rows > 0;
                $checkStmt->close();

                if ($hasSecretary) {
                    $errors[] = 'سڕینەوە ڕاگیرا: سەرەتا سکرتێرە پەیوەندیدارەکان بسڕەوە';
                } else {
                    $deleteStmt = $conn_kasher_platform->prepare("
                        DELETE FROM doctors
                        WHERE id = ? AND user_id = ?
                    ");
                    $deleteStmt->bind_param('ii', $doctorId, $userId);
                    if ($deleteStmt->execute()) {
                        setMessage('پزیشک بە سەرکەوتوویی سڕایەوە', 'success');
                        $deleteStmt->close();
                        redirect(url('user/medical_staff/doctors.php'));
                    }
...
<head>
    ...
    <title>پزیشکەکان - <?php echo SITE_NAME; ?></title>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
    <div class="container-fluid">
        <a class="navbar-brand" href="<?php echo url('user/medical_staff/main.php'); ?>"><i class="bi bi-hospital"></i> پزیشکەکان</a>
...
<div class="container-fluid py-4">
    ...
    <div class="card mb-4 shadow-sm">
        <div class="card-body">
            <h5 class="card-title mb-3">زیادکردنی پزیشک</h5>
            <form method="post" class="row g-3">
                <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8'); ?>">
                <input type="hidden" name="action" value="<?php echo $editDoctorId > 0 ? 'update' : 'add'; ?>">
                <input type="hidden" name="doctor_id" value="<?php echo (int)$editDoctorId; ?>">
                <div class="col-md-3">
                    <label class="form-label">ناوی پزیشک</label>
                    <input type="text" name="name" class="form-control" required value="<?php echo htmlspecialchars($doctorForm['name'], ENT_QUOTES, 'UTF-8'); ?>">
                </div>
                <div class="col-md-3">
                    <label class="form-label">ئیمەیڵ</label>
                    <input type="email" name="email" class="form-control" required value="<?php echo htmlspecialchars($doctorForm['email'], ENT_QUOTES, 'UTF-8'); ?>">
                </div>
                <div class="col-md-3">
                    <label class="form-label">پاسۆرد <?php if ($editDoctorId > 0): ?><small class="text-muted">(ئەگەر خالی بێت، ناگۆڕدرێت)</small><?php endif; ?></label>
                    <input type="password" name="password" class="form-control" minlength="6" <?php echo $editDoctorId > 0 ? '' : 'required'; ?>>
                </div>
                <div class="col-md-3">
                    <label class="form-label">ژمارەی مۆبایل</label>
                    <input type="text" name="mobile" class="form-control" required value="<?php echo htmlspecialchars($doctorForm['mobile'], ENT_QUOTES, 'UTF-8'); ?>">
                </div>
                <div class="col-12">
                    <button type="submit" class="btn btn-primary">
                        <i class="bi <?php echo $editDoctorId > 0 ? 'bi-check2-circle' : 'bi-plus-circle'; ?>"></i>
                        <?php echo $editDoctorId > 0 ? 'نوێکردنەوە' : 'زیادکردن'; ?>
                    </button>
                    <?php if ($editDoctorId > 0): ?>
                        <a href="<?php echo url('user/medical_staff/doctors.php'); ?>" class="btn btn-outline-secondary">هەڵوەشاندنەوە</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <h5 class="card-title mb-3">لیستی پزیشکەکان</h5>
            <div class="table-responsive">
                <table class="table table-striped align-middle">
                    ...
                    <?php if (empty($doctors)): ?>
                        <tr><td colspan="5" class="text-center text-muted py-4">هێشتا هیچ پزیشکێک تۆمار نەکراوە</td></tr>
                    <?php else: ?>
                        <?php foreach ($doctors as $doctor): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($doctor['name'], ENT_QUOTES, 'UTF-8'); ?></td>
                                <td><?php echo htmlspecialchars($doctor['email'], ENT_QUOTES, 'UTF-8'); ?></td>
                                <td><?php echo htmlspecialchars($doctor['mobile'], ENT_QUOTES, 'UTF-8'); ?></td>
                                <td><?php echo htmlspecialchars((string)$doctor['created_at'], ENT_QUOTES, 'UTF-8'); ?></td>
                                <td class="text-center">
                                    <div class="d-flex gap-2 justify-content-center">
                                        <a href="<?php echo url('user/medical_staff/doctors.php?edit=' . (int)$doctor['id']); ?>" class="btn btn-sm btn-outline-primary">
                                            <i class="bi bi-pencil-square"></i> دەستکاری
                                        </a>
                                        <form method="post" onsubmit="return confirm('دڵنیایت لە سڕینەوەی ئەم پزیشکە؟');">
                                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8'); ?>">
