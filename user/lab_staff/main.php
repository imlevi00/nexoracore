...
$labStmt = $conn_kasher_platform->prepare("
    SELECT id FROM medical_center_labs
    WHERE user_id = ?
    LIMIT 1
");
if ($labStmt) {
    ...
    $labStmt->execute();
    $hasLabAccount = $labStmt->get_result()->num_rows > 0;
    $labStmt->close();
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>بەڕێوەبردنی تاقیگە - <?php echo SITE_NAME; ?></title>
    ...
    <style>
        ...
        .section-lab-create { background: linear-gradient(135deg, #0284c7, #0369a1); }
        .section-lab-manage { background: linear-gradient(135deg, #0891b2, #0e7490); }
        .section-lab-login { background: linear-gradient(135deg, #6366f1, #4338ca); }
...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
    <div class="container-fluid">
        <a class="navbar-brand" href="<?php echo url('user/dashboard/index.php'); ?>">
            <i class="bi bi-flask"></i> <?php echo htmlspecialchars($currentUser['business_name'], ENT_QUOTES, 'UTF-8'); ?>
        </a>
...
<div class="container-fluid py-4">
    <div class="row mb-4">
        <div class="col-12">
            <h2 class="mb-1">بەڕێوەبردنی تاقیگە</h2>
            <p class="text-muted mb-0">
                <?php if ($hasLabAccount): ?>
                    دەستکاری ئەکاونتی تاقیگە یان چوونەژوورەوە بۆ پۆرتاڵی تاقیگە
                <?php else: ?>
                    دروستکردنی ئەکاونتی تاقیگە (تەنها یەک ئەکاونت بۆ هەر کارگە)
                <?php endif; ?>
            </p>
        </div>
    </div>
    ...
    <div class="row g-3">
        <?php if (!$hasLabAccount): ?>
            <div class="dashboard-card-wrapper p-2">
                <a href="<?php echo url('user/lab_staff/account.php'); ?>" class="text-decoration-none">
                    <div class="dashboard-card section-lab-create p-4 h-100">
                        <div class="text-center text-white">
                            <div class="card-icon"><i class="bi bi-plus-circle"></i></div>
                            <h5 class="mb-1">دروستکردنی ئەکاونتی تاقیگە</h5>
                            <p class="mb-0 small">تەنها یەک ئەکاونت دەتوانرێت دروست بکرێت</p>
                        </div>
                    </div>
                </a>
            </div>
        <?php else: ?>
            <div class="dashboard-card-wrapper p-2">
                <a href="<?php echo url('user/lab_staff/account.php'); ?>" class="text-decoration-none">
                    <div class="dashboard-card section-lab-manage p-4 h-100">
                        <div class="text-center text-white">
                            <div class="card-icon"><i class="bi bi-pencil-square"></i></div>
                            <h5 class="mb-1">دەستکاری ئەکاونتی تاقیگە</h5>
                            <p class="mb-0 small">نوێکردنەوەی ناو، ئیمەیڵ، مۆبایل و پاسۆرد</p>
                        </div>
                    </div>
                </a>
            </div>
            ...
            <div class="dashboard-card-wrapper p-2">
                <a href="<?php echo url('professions/medical-center/lab/dashboard'); ?>" class="text-decoration-none">
                    <div class="dashboard-card section-lab-login p-4 h-100">
                        <div class="text-center text-white">
                            <div class="card-icon"><i class="bi bi-box-arrow-in-right"></i></div>
                            <h5 class="mb-1">چوونەژوورەوە بۆ تاقیگە</h5>
                            <p class="mb-0 small">چوونە ناو پۆرتاڵی تاقیگە لە medical center</p>
                        </div>
                    </div>
                </a>
            </div>
