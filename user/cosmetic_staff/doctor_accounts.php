    ...
    if (!Security::validateCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($action === 'add' || $action === 'update') {
            ...
            if ($name === '' || $email === '' || $mobile === '') {
                $errors[] = 'تکایە ناو، ئیمەیڵ و مۆبایل پڕبکەرەوە';
            }
            ...
            if (empty($errors)) {
                if ($action === 'add') {
                    ...
                    $insertStmt = $conn_kasher_platform->prepare("
                        INSERT INTO cosmetic_doctor_accounts (user_id, name, email, password_hash, mobile, created_at, updated_at)
                        VALUES (?, ?, ?, ?, ?, NOW(), NOW())
                    ");
                    if ($insertStmt) {
                        $insertStmt->bind_param('issss', $userId, $name, $email, $passwordHash, $mobile);
                        if ($insertStmt->execute()) {
                            setMessage('ئەکاونتی دکتۆر دروستکرا', 'success');
                            $insertStmt->close();
                            redirect(url('user/cosmetic_staff/doctor_accounts.php'));
                        }
                        if ((int)$conn_kasher_platform->errno === 1062) {
                            $errors[] = 'ئەم ئیمەیڵە پێشتر تۆمارکراوە';
                        } else {
                            $errors[] = 'هەڵە لە دروستکردن';
                        }
                        $insertStmt->close();
                    } else {
                    ...
                    if ($accountId <= 0) {
                        ...
                        if ($password !== '') {
                            ...
                            $updateStmt = $conn_kasher_platform->prepare("
                                UPDATE cosmetic_doctor_accounts
            ...
            if ($accountId <= 0) {
                ...
                $deleteStmt = $conn_kasher_platform->prepare("
                    DELETE FROM cosmetic_doctor_accounts
                    WHERE id = ? AND user_id = ?
                ");
                $deleteStmt->bind_param('ii', $accountId, $userId);
                if ($deleteStmt->execute()) {
                    setMessage('ئەکاونتی دکتۆر سڕایەوە', 'success');
                    $deleteStmt->close();
                    redirect(url('user/cosmetic_staff/doctor_accounts.php'));
                }
...
<head>
    ...
    <title>دکتۆری جوانکاری - <?php echo SITE_NAME; ?></title>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
    <div class="container-fluid">
        <a class="navbar-brand" href="<?php echo url('user/cosmetic_staff/main.php'); ?>"><i class="bi bi-person-badge"></i> دکتۆری جوانکاری</a>
...
<div class="container-fluid py-4">
    ...
    <div class="card mb-4 shadow-sm">
        <div class="card-body">
            <h5 class="card-title mb-3"><?php echo $editId > 0 ? 'دەستکاری ئەکاونتی دکتۆر' : 'ئەکاونتی دکتۆری نوێ'; ?></h5>
            <form method="post" class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <?php if ($editId > 0): ?><a href="<?php echo url('user/cosmetic_staff/doctor_accounts.php'); ?>" class="btn btn-outline-secondary">هەڵوەشاندنەوە</a><?php endif; ?>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <h5 class="card-title mb-3">لیستی دکتۆرەکان</h5>
                    ...
                    <?php else: foreach ($rows as $r): ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="<?php echo url('user/cosmetic_staff/doctor_accounts.php?edit=' . (int)$r['id']); ?>" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil-square"></i></a>
