<?php
/**
 * Remove A4 Receipt Banner - user/settings/remove_a4_banner.php
 * سڕینەوەی وێنەی بانەری وەسڵی A4
 */
...
try {
    ...
    $stmt = $conn->prepare("SELECT a4_receipt_banner FROM settings WHERE user_id = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $bannerFile = $settings['a4_receipt_banner'];
        ...
        if ($bannerFile) {
            ...
            $updateStmt = $conn->prepare("
                UPDATE settings 
                SET a4_receipt_banner = NULL, updated_at = NOW() 
                WHERE user_id = ?
            ");
